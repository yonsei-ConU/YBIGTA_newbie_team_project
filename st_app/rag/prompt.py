...
def create_review_summary_prompt(query: str, context: str) -> str:
    ...
    review_count = len(context.split('[')) - 1
    
    return f"""다음은 영화 리뷰 데이터입니다:
...
답변 형식:
- 리뷰 분석 결과
- 검색된 리뷰 정보 (분석한 리뷰 수: {review_count}개)
- 주요 키워드 (리뷰에서 실제 언급된 키워드들)
- 상세 분석 (제공된 리뷰 데이터만을 바탕으로 한 분석)

**답변 요구사항:**
1. 자연스럽고 친근한 톤으로 답변해주세요
2. 이전 대화 맥락을 고려하여 연속성 있게 답변해주세요
3. 구체적인 예시를 들어가며 설명해주세요
4. 사용자가 실제로 도움받을 수 있는 정보를 제공해주세요
5. 리뷰의 핵심 내용을 요약해서 보여주세요
6. 한국어로 답변해주세요
7. 답변 길이는 2-3문단 이내로 간결하게 작성해주세요
